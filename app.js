...
async function onSubmitForm(event) {
  ...
  try {
    const docRef = state.selectedId ? doc(db, COLLECTION_NAME, state.selectedId) : doc(collectionRef);
    await setDoc(
      docRef,
      {
        name,
    ...
    setStatus(state.selectedId ? `Đã lưu "${name}" lên Firebase.` : `Đã thêm "${name}".`, "success");
  } catch (error) {
